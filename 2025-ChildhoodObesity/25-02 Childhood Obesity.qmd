...
topau_0413_5list <- topau_0413 %>% 
  head(n = 5) %>% 
  pull(std_name)

oriau_byti2 %>% 
  filter(std_name %in% topau_0413_5list) %>% 
  arrange(desc(PY)) %>% 
  distinct(std_name, .keep_all = T) %>% 
  mutate(short_name = str_sub(std_name, 1, 10), 
         short_ti = str_sub(TI, 1, 40)) %>% 
...
topau_1423_5list <- topau_1423 %>% 
  head(n = 5) %>% 
  pull(std_name)

oriau_byti2 %>% 
  filter(std_name %in% topau_1423_5list) %>% 
  arrange(desc(PY)) %>% 
  distinct(std_name, .keep_all = T) %>% 
  mutate(short_name = str_sub(std_name, 1, 10), 
         short_ti = str_sub(TI, 1, 40)) %>% 
...
```{r}
topau_9403_15 <- oriau_byti2 %>% 
  filter(PY %in% 1994:2003) %>% 
  count(std_name, sort = T) %>% 
  slice_head(n = 15) %>% 
  mutate(rank = row_number(), .before = 1)

topau_0413_15 <- oriau_byti2 %>% 
  filter(PY %in% 2004:2013) %>% 
  count(std_name, sort = T) %>% 
  slice_head(n = 15) %>% 
  mutate(rank = row_number(), .before = 1)

topau_1423_15 <- oriau_byti2 %>% 
  filter(PY %in% 2014:2023) %>% 
  count(std_name, sort = T) %>% 
  slice_head(n = 15) %>% 
  mutate(rank = row_number(), .before = 1)
...
topau_9403_15 %>% 
  left_join(topau_0413_15, by = "rank") %>% 
  left_join(topau_1423_15, by = "rank") #%>% 
  #writexl::write_xlsx("top15author.xlsx")
```
